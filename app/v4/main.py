...
@app.post("/reverse-transform")
async def reverse_transform_message(request: TransformationRequest):
    """
    Reverse transform MX/XML message to MT format using reverse orchestrator
    """
    logger.info(f"📥 Reverse transformation request received - Approach: {request.approach}")
    
    try:
        from services import reverse_orchestrator
        
        # Create transformation state
        state = TransformationState(
            raw_mt_message=request.mt_message,  # Contains XML
            message_id=request.message_id,
            approach=request.approach
        )
        
        # Execute reverse transformation
        result = await reverse_orchestrator.transform(state)
        
        if not result.get("success"):
            error_msg = result.get("error", "Reverse transformation failed")
            logger.error(f"Reverse transformation failed: {error_msg}")
            raise HTTPException(
                status_code=500,
                detail=error_msg
            )
        ...
        # Build response
        parsed_xml = result.get("parsed_xml", {})
        mt_output = result.get("mt_output", {})
        agent_results = result.get("agent_results", {})
        
        statistics = {
            "total_duration_ms": result.get("total_duration_ms", 0),
            "fields_extracted": parsed_xml.get("field_count", 0),
            "mt_message_generated": bool(mt_output.get("message", ""))
        }
        
        response_data = {
            "success": True,
            "message_id": parsed_xml.get("message_id", "unknown"),
            "mx_type": parsed_xml.get("message_type", "XXX"),
            "mt_type": mt_output.get("mt_type", "MTXXX"),
            "approach_used": request.approach.value,
            "parsed_xml": parsed_xml,
            "mt_message": mt_output.get("message", ""),
            "mt_fields": mt_output.get("field_count", 0),
            "agent_results": agent_results,
            "statistics": statistics,
            "timestamp": datetime.utcnow().isoformat()
        }
        
        logger.info(f"✅ Reverse transformation complete - {response_data['message_id']}")
        return response_data
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"❌ Reverse transformation error: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
