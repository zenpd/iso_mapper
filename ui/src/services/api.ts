...
class APIClient {
  ...
  async reverseTransform(request: TransformationRequest): Promise<TransformationResponse> {
    const response = await this.client.post<TransformationResponse>(
      '/api/v1/reverse-transform',
      request
    )
    return response.data
  }
  ...
  async validateTransformation(request: TransformationRequest): Promise<ValidationResult> {
    const response = await this.client.post<ValidationResult>(
      '/api/v1/validate-transformation',
      request
    )
    return response.data
  }
